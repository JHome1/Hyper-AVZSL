...
def Sim_matrix(a, b):
    a_norm, b_norm = a/a.norm(dim=1)[:, None], b/b.norm(dim=1)[:, None]
...
class AVCA(nn.Module):
    def __init__(self, params_model, input_size_audio, input_size_video):
        ...
        self.optimizer_gen = optim.Adam(list(self.A_proj.parameters()) + list(self.V_proj.parameters()) +
                                        ...
                                        list(self.W_proj.parameters()), lr=self.lr, weight_decay=1e-5)
...
class Hyper_AVCA(nn.Module):
    def __init__(self, params_model, input_size_audio, input_size_video, args):
        super(Hyper_AVCA, self).__init__()
        ...
        ################################
        self.args = args
        self.align_loss = Alignment_loss()
    
        if self.args.hyper_framework=='Hyper-alignment':
            self.curvature = self.args.curvature

        if self.args.hyper_framework=='Hyper-single':
            self.scale =self.args.curvature
            self.fusenet1 = nn.Linear(in_features=600, out_features=1)
            self.fusenet2 = nn.Sigmoid()

        if self.args.hyper_framework=='Hyper-multiple':
            self.scale =self.args.curvature
            self.fusenet1 = nn.Linear(in_features=600, out_features=self.args.num_curvature)
            self.fusenet2 = nn.Sigmoid()
        ################################
        ...
        ###################################################################
        if self.args.hyper_framework in {'Hyper-single', 'Hyper-multiple'}:
            self.optimizer_gen = optim.Adam(list(self.A_proj.parameters()) + list(self.V_proj.parameters()) +
                                            list(self.A_rec.parameters()) + list(self.V_rec.parameters()) +
                                            list(self.V_enc.parameters()) + list(self.A_enc.parameters()) +
                                            list(self.cross_attention.parameters()) + list(self.D.parameters()) +
                                            list(self.W_proj.parameters()) + 
                                            list(self.fusenet1.parameters()),
    ...
    def backward(self, optimize):
        ################################################
        if self.args.hyper_framework=='Hyper-alignment':
            self.phi_a_h = hypmath.logmap0(hypmath.project(self.phi_a, c=self.curvature), c=self.curvature)
            self.phi_a_neg_h = hypmath.logmap0(hypmath.project(self.phi_a_neg, c=self.curvature), c=self.curvature)

            self.phi_v_h = hypmath.logmap0(hypmath.project(self.phi_v, c=self.curvature), c=self.curvature)
            self.phi_v_neg_h = hypmath.logmap0(hypmath.project(self.phi_v_neg, c=self.curvature), c=self.curvature)
        
            l_align = self.align_loss(self.phi_v_h, self.phi_a_h) + \
                      self.align_loss(self.phi_v_neg_h, self.phi_a_neg_h)
            loss_align = 1e-3*l_align

        if self.args.hyper_framework=='Hyper-single':
            curvature = self.fusenet1(torch.cat((self.phi_v, self.phi_a), 1))
            curvature = self.scale*self.fusenet2(curvature)

            curvature_neg = self.fusenet1(torch.cat((self.phi_v_neg, self.phi_a_neg), 1))
            curvature_neg = self.scale*self.fusenet2(curvature_neg)

            self.phi_a_h = hypmath.logmap0(hypmath.project(self.phi_a, c=curvature), c=curvature)
            self.phi_a_neg_h = hypmath.logmap0(hypmath.project(self.phi_a_neg, c=curvature_neg), c=curvature_neg)

            self.phi_v_h = hypmath.logmap0(hypmath.project(self.phi_v, c=curvature), c=curvature)
            self.phi_v_neg_h = hypmath.logmap0(hypmath.project(self.phi_v_neg, c=curvature_neg), c=curvature_neg)
        ...
        if self.args.hyper_framework=='Hyper-multiple':
             ...
             l_align = 0
             for i in range(self.args.num_curvature):
                 self.phi_a_h_ele = hypmath.logmap0(hypmath.project(self.phi_a, c=curvature[:, i].unsqueeze(-1)), c=curvature[:, i].unsqueeze(-1))
                 self.phi_a_neg_h_ele = hypmath.logmap0(hypmath.project(self.phi_a_neg, c=curvature_neg[:, i].unsqueeze(-1)), c=curvature_neg[:, i].unsqueeze(-1))
                 self.phi_v_h_ele = hypmath.logmap0(hypmath.project(self.phi_v, c=curvature[:, i].unsqueeze(-1)), c=curvature[:, i].unsqueeze(-1))
                 self.phi_v_neg_h_ele = hypmath.logmap0(hypmath.project(self.phi_v_neg, c=curvature_neg[:, i].unsqueeze(-1)), c=curvature_neg[:, i].unsqueeze(-1))

                 l_align_ele = self.align_loss(self.phi_v_h_ele, self.phi_a_h_ele) + \
                               self.align_loss(self.phi_v_neg_h_ele, self.phi_a_neg_h_ele)                 
                 l_align = l_align+l_align_ele

             l_align = l_align/self.args.num_curvature
             loss_align = 1e-3*l_align
        ################################################
